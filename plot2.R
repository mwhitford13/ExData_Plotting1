...
Filtered_HPC_Data$datetime <- strptime(paste(Filtered_HPC_Data$Date, Filtered_HPC_Data$Time, sep=" "), "%d/%m/%Y %H:%M:%S") 

#Create Plot
png("plot2.png", width=480, height=480)
plot(Filtered_HPC_Data$datetime,as.numeric(as.character(Filtered_HPC_Data$Global_active_power)),type="l",xlab="",ylab="Global Active Power (kilowatts)") 
dev.off() 
